...
fun FoodDeliveryApp() {
    ...
    val favoritesViewModel = FavoritesViewModel(context) // Передаем context
    ...
    Column(
        ...
        Box(modifier = Modifier.weight(1f)) {
            ...
            NavHost(navController = navController, startDestination = "home") {
                composable("home") { HomeScreen(mainViewModel, navController, favoritesViewModel) } // Обновлено
                composable("catalog") { CatalogScreen(catalogViewModel, navController) }
                composable("favorites") { FavoritesScreen(favoritesViewModel) } // Передаем favoritesViewModel
